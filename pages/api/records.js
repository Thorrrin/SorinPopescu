...
const addDonation = async (id) => {
	const collection = await getCollection(COLLECTION_NAME);
	let record = await collection.findOne({_id: new ObjectId(id)});
	delete record._id;
	const donationAmount = 100;
	if(record.donation===undefined) {
		record.donation = 0;
	}
	record.donation += donationAmount;
	console.log(record);
	const col2 = await collection.updateOne({_id: new ObjectId(id)}, {$set: record});
	return collection.find({}).toArray();
}

export default async function handler(req, res) {

	const isAllowedMethod = req.method === 'GET' || req.method === 'POST' || req.method === 'PUT' || req.method === 'DELETE' || req.method === 'PATCH';
	if(!isAllowedMethod) {
	...
	if(req.method === 'GET' && req.query.id) {
		...
		const id = req.query.id;
		const result = await addDonation(id);
		return sendOk(res, result);
	}
}